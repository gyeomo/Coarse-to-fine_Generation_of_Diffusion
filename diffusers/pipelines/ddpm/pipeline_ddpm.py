...
def find_nearest(array, value):
    # Find the position of an element close to a specific value in an array (ICCV2023)
...
class DDPMPipeline(DiffusionPipeline):
    def __init__(self, unet, scheduler):
        ...
        # Range Configuration based on Noise Schedules (ICCV2023)
    ...
    def adjust_weight(self, stats):
        # Make the  \hat{h}(t) (ICCV2023)
        # Find the time step point where \hat{h}(t) reaches 0 (ICCV2023)
        h_list = []
        for j in stats:
    ...
    def __call__(self,criteria=None, condition=None, xT=None, weight = None, batch_size=1, generator=None, torch_device=None, output_type="pil"):
        ...
        if xT is not None:
            image = xT
        xT = image.clone() # Save for reproduction (ICCV2023)
            
        image = image.to(torch_device)
        if condition is not None:
            # Range Adjustment based on Latent Features (ICCV2023)
            weight = weight
        ...
        for t in tqdm(self.scheduler.timesteps):
            if condition is None:
                # Common reverse process (ICCV2023)
                model_output = self.unet(image, t)
                h = model_output["h"] # Save for manipulating (ICCV2023)
                model_output = model_output["sample"]
                process = self.scheduler.step(model_output, t, image)
            else:
                # ------------------------------------------------------------------------------
                if criteria == 0: # Coarse editing (ICCV2023)
                    if t >= coarse:
                        # Manipulating process (ICCV2023)
                        model_output = self.unet(image, t, [condition[1][t][1],0.75])
                    else:
                        # Common reverse process (ICCV2023)
                        model_output = self.unet(image, t)
                        
                elif criteria == 1: # Medium editing (ICCV2023)
                ...
                elif criteria == 2: # Fine editing (ICCV2023)
                    if t <= fine and t > 10:
                        # Manipulating process (ICCV2023)
                        model_output = self.unet(image, t, [condition[1][t][1],0.75])
                    else:
            ...
            noise = process["noise"] # Save for reproduction (ICCV2023)
