    ...
    def step(
        ...
        if t > 0:
            if condition is not None:
                noise = condition[0] # Set to the noise used when generating the source image (ICCV2023)
            else:
                noise = self.randn_like(model_output, generator=generator)
            variance = (self._get_variance(t, predicted_variance=predicted_variance) ** 0.5) * noise
        else:
            noise = torch.zeros(model_output.shape)
        pred_prev_sample = pred_prev_sample + variance

        return {"prev_sample": pred_prev_sample, "noise":noise}
